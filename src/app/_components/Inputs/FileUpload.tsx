...
const FileUpload = forwardRef<HTMLInputElement, Props>(
  ({ name, label, required, placeholder, value, setValue }, ref) => {
    return (
      <div className="flex flex-col gap-1">
        ...
        <label
          htmlFor={name}
          className={`flex gap-2 items-center justify-center rounded-full w-full px-7 brutal-sm cursor-pointer ${
            value ? "mb-1" : "mb-3"
          }`}
        >
          <DocumentIcon className="size-5 -ml-1 text-neutral-600" />
          {value ? (
        ...
        <input
          ...
          onChange={(e) =>
            setValue && setValue(e.target.files ? e.target.files[0] : null)
          }
          ref={ref}
        />
      </div>
