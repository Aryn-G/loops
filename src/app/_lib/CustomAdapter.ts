...
import mongoose, { Models, Mongoose } from "mongoose";
...
export interface MongoDBAdapterOptions {
  ...
  collections: {
    ...
    VerificationTokens: mongoose.Model<any>;
  };
  /**
...
export function CustomAdaptor(
  ...
  return {
    async createUser(data) {
      const user2 = to<AdapterUser>(data);
      // await using db = await getDb()
      await mongoDB();
      // console.log("Trying to create: ", data)
      const user = new collections.Users(user2);
      await user.save();
    ...
    async getUser(id) {
      ...
      const user = await collections.Users.findById(id);
      console.log("Trying to find: ", id, ", User: ", user);
    ...
    async getUserByEmail(email) {
      ...
      console.log("Trying to find: ", email, ", User: ", user);
      if (!user) return null;
      return from<AdapterUser>(user);
    },
    async getUserByAccount(provider_providerAccountId) {
      ...
      const user = await collections.Users.findById(account.userId);
      console.log("Trying to find: ", account.userId, ", User: ", user);
    ...
    async updateUser(data) {
      ...
      const result = await collections.Users.findByIdAndUpdate(
        _id,
        { $set: user },
    ...
    async deleteUser(id) {
      ...
      await Promise.all([
        collections.Accounts.deleteMany({ userId: id }),
        collections.Sessions.deleteMany({ userId: id }),
        collections.Users.findByIdAndDelete(id),
      ]);
    },
    linkAccount: async (data) => {
      const account2 = to<AdapterAccount>(data);
      // await using db = await getDb()
      await mongoDB();
      console.log("Trying to Link Account: ", data);
      const account = new collections.Accounts(account2);
      await account.save();
    ...
    async unlinkAccount(provider_providerAccountId) {
      // await using db = await getDb()
      await mongoDB();
      const account = await collections.Accounts.findOneAndDelete(
        provider_providerAccountId
      );
      return from<AdapterAccount>(account!);
    },
    async getSessionAndUser(sessionToken) {
      ...
      const session = await collections.Sessions.findOne({ sessionToken });
      if (!session) return null;

      const user = await collections.Users.findById(session.userId);
      if (!user) return null;
      return {
    ...
    async createSession(data) {
      ...
      // await using db = await getDb()
      await mongoDB();
      const s = new collections.Sessions(session);
      await s.save();
    ...
    async updateSession(data) {
      ...
      const updatedSession = await collections.Sessions.findOneAndUpdate(
    ...
    async deleteSession(sessionToken) {
      ...
      const session = await collections.Sessions.findOneAndDelete({
        sessionToken,
      });
      return from<AdapterSession>(session!);
    },
    async createVerificationToken(data) {
      ...
      const s = new collections.VerificationTokens(to(data));
      await s.save();
      return data;
    },
    async useVerificationToken(identifier_token) {
      ...
      const verificationToken =
        await collections.VerificationTokens.findOneAndDelete(identifier_token);
